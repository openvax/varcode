...
def test_timing_load(n_loci=500, profile=False):
    loci = [
...
def test_timing_group_by_allele(n_iterations=12, profile=False):
    locus = Locus.from_interbase_coordinates("20", 29628224)
    variant = Variant(locus, "A", "G")
    #data = data_path("reads/gatk_mini_bundle_extract.bam")
    data = "/Users/tim/mount/demeter/demeter/users/odonnt02/data/pt189-ovarian/all/189/Illumina_DNA/PT189_Right_Ovary_L1/Processed/NGS.2_7_2b.WES/results/PT189_Right_Ovary_L1.PT189_Right_Ovary_L1.clean.dedup.recal.bam"
    print("Loading")
    evidence = PileupCollection.from_bam(data, [locus])
    print("Done")

    elapsed = -time.time()
    for i in range(n_iterations):
        #evidence.group_by_allele(locus)
        print(evidence.match_summary(variant))

    elapsed += time.time()

    print("Time to test %d groupby operations: %f" % (
